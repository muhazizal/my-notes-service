...
describe('auth integration', () => {
  ...
  test('register -> resend verification -> verify -> login -> logout -> check session', async () => {
    ...
    const body = { email, password: 'Passw0rd!', username: 'user1', fullname: 'User One' }
  ...
  test('forgot-password -> reset-password -> login with new password', async () => {
    ...
    const body = { email, password, username: 'resetuser', fullname: 'Reset User' }
    ...
    const reset = await agent.post(`/api/auth/reset-password/${resetToken}`).send({ password: newPassword })
